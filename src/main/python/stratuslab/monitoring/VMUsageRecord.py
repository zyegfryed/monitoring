...
class VMUsageRecord(object):
    ...
    def _dom_usage_record(dom):
        ...
        """
        (record, disks, interfaces) = VMUsageRecord._dom_xml_info(dom)
        record.update(VMUsageRecord._dom_disk_io(dom, disks))
        record.update(VMUsageRecord._dom_network_io(dom, interfaces))
        record.update(VMUsageRecord._dom_state_cpu_memory(dom))
    ...
    def usage_record(self, uuidstr):
        ...
        return VMUsageRecord._dom_usage_record(dom)
    ...
    def all_usage_records(self):
        ...
        return [VMUsageRecord._dom_usage_record(dom) for dom in doms]
